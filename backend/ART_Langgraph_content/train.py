...
# ----------------- 奖励函数 -----------------
def _collect_text_nodes(task: List[Dict[str, Any]]) -> List[Tuple[List[int], str]]:
    """返回所有 text 节点的位置和内容：[(path_index_chain, text), ...]。"""
    out = []
    for i, node in enumerate(task):
        t = node.get("type")
        data = node.get("data", {})
        if t == "content" and isinstance(data.get("items"), list):
            for j, it in enumerate(data["items"]):
                if isinstance(it, dict) and "text" in it:
                    out.append(([i, j], it.get("text", "")))
        elif t in ("cover", "transition"):
            # 这些通常没有 text 节点需要填充；如有可在此扩展
            pass
    return out

def _structure_match_score(inp: List[Dict[str, Any]], outp: List[Dict[str, Any]]) -> float:
    """检查结构一致性：长度、type、title、items 长度、item.title 等。"""
    try:
        if not isinstance(outp, list) or len(inp) != len(outp):
            return 0.0
        score = 0.0
        total = 0.0
        for i, (a, b) in enumerate(zip(inp, outp)):
            total += 1
            if a.get("type") != b.get("type"):
                continue
            at, bt = a.get("type"), b.get("type")
            # data.title
            adt, bdt = a.get("data", {}), b.get("data", {})
            if at in ("cover", "transition"):
                total += 1
                score += 1 if adt.get("title") == bdt.get("title") else 0
            if at == "content":
                total += 2
                score += 1 if adt.get("title") == bdt.get("title") else 0
                aitems, bitems = adt.get("items", []), bdt.get("items", [])
                if isinstance(aitems, list) and isinstance(bitems, list) and len(aitems) == len(bitems):
                    score += 1
                    # 校验 item.title
                    for ai, bi in zip(aitems, bitems):
                        total += 1
                        score += 1 if ai.get("title") == bi.get("title") else 0
        return score / max(total, 1.0)
    except Exception:
        return 0.0

def _citation_coverage(texts: List[str]) -> float:
    """有 [n] 引用的文本占比。"""
    if not texts:
        return 0.0
    hits = sum(1 for t in texts if re.search(r"\[\d+\]", str(t)))
    return hits / len(texts)

def _numeric_density(texts: List[str]) -> float:
    """包含数字/百分比/年份等的占比。"""
    if not texts:
        return 0.0
    pat = re.compile(r"(\d{4}|[0-9]+(?:\.[0-9]+)?%|\b\d+(?:\.\d+)?\b)")
    hits = sum(1 for t in texts if pat.search(str(t)))
    return hits / len(texts)
...
def format_reward(inp_task: List[Dict[str, Any]], out_task: List[Dict[str, Any]]) -> float:
    """0~1：结构一致 + 文本完整 + 引用覆盖 + JSON 合法。"""
    try:
        # 结构一致性 40%
        s_score = _structure_match_score(inp_task, out_task)
        # 文本完整 30%（长度≥120 且非默认模板）
        out_texts = [t for _, t in _collect_text_nodes(out_task)]
        if not out_texts:
            return 0.0
        filled = 0
        for t in out_texts:
            tt = (t or "").strip()
            if len(tt) >= 120 and not tt.startswith("Detailed content"):
                filled += 1
        fill_rate = filled / len(out_texts)
        # 引用覆盖 20%
        cite_rate = _citation_coverage(out_texts)
        # JSON 合法 10%（能 json.dumps 成功即视为合法）
        try:
            json.dumps(out_task, ensure_ascii=False)
            json_ok = 1.0
        except Exception:
            json_ok = 0.0
        return 0.40 * s_score + 0.30 * fill_rate + 0.20 * cite_rate + 0.10 * json_ok
    except Exception:
        return 0.0

def search_reward(out_task: List[Dict[str, Any]], sources: List[str],
                  tool_urls_seen: Optional[List[str]] = None) -> float:
    """0~1：来源多样性 + 具体化 + 引用一致性 + 新近性启发 + （可选）来源来自工具。"""
    sources = [s for s in (sources or []) if isinstance(s, str) and s.startswith("http")]
    src_count = len(set(sources))
    if src_count == 0:
        return 0.0

    # 多样性：以域名去重后的比例做一个 0~1 分
    domains = { _url_domain(u) for u in sources if _url_domain(u) }
    diversity = min(len(domains) / max(src_count,1), 1.0)

    texts = [t for _, t in _collect_text_nodes(out_task)]
    numeric = _numeric_density(texts)

    # 引用一致性：所有 [n] 均映射到 sources 有效索引
    def _all_citations_valid() -> float:
        ok, total = 0, 0
        for t in texts:
    ...
    cite_valid = _all_citations_valid()

    # 新近性启发：出现 2023/2024/2025/2026 或季度/月份
    freshness = 1.0 if re.search(r"(2023|2024|2025|2026|Q[1-4]|20\d{2}年|\d+月)", " ".join(texts)) else 0.0

    # （可选）工具一致性：sources 必须是工具里看过的 URL 子集
    tool_ok = 1.0
    if tool_urls_seen:
        tool_set = set(tool_urls_seen)
        if tool_set:
            in_tool = [u for u in sources if u in tool_set]
            tool_ok = len(in_tool) / len(sources)

    # 权重：多样性0.35 + 具体化0.35 + 引用一致0.20 + 新近性0.10，再乘以工具一致性
    base = 0.35 * diversity + 0.35 * numeric + 0.20 * cite_valid + 0.10 * freshness
    return base * tool_ok
