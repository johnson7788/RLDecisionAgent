...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default="char_count")
    ...
    print("\n📄 SFT 训练集样本（前 3 条）：")
    for i in range(min(3, len(sft_train_dataset))):
        print(f"[样本 {i + 1}]")
        print("Prompt:", sft_train_dataset.iloc[i]['prompt'])
        print("Response:\n", sft_train_dataset.iloc[i]['response'])
        print("-" * 50)

    print("\n📄 SFT 测试集样本（前 3 条）：")
    for i in range(min(3, len(sft_test_dataset))):
        print(f"[样本 {i + 1}]")
        print("Prompt:", sft_test_dataset.iloc[i]['prompt'])
        print("Response:\n", sft_test_dataset.iloc[i]['response'])
        print("-" * 50)
    ...
    print("\n📄 RL 训练集样本（前 3 条）：")
    for i in range(min(3, len(rl_train_dataset))):
        print(f"[样本 {i + 1}]")
        print("Prompt:", rl_train_dataset.iloc[i]['prompt'])
        print("Reward model:", rl_train_dataset.iloc[i]['reward_model'])
        print("Extra info (response):\n", rl_train_dataset.iloc[i]['extra_info']['response'])
        print("-" * 50)

    print("\n📄 RL 测试集样本（前 3 条）：")
    for i in range(min(3, len(rl_test_dataset))):
        print(f"[样本 {i + 1}]")
        print("Prompt:", rl_test_dataset.iloc[i]['prompt'])
        print("Reward model:", rl_test_dataset.iloc[i]['reward_model'])
        print("Extra info (response):\n", rl_test_dataset.iloc[i]['extra_info']['response'])
        print("-" * 50)
